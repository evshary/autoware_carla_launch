autoware_carla_launch
=====================

The package includes launch file to run Autoware, Carla agent, and bridges.

* `zenoh_carla_bridge <https://github.com/evshary/zenoh_carla_bridge>`_
* `rmw_zenoh <https://github.com/ros2/rmw_zenoh>`_
* `zenoh-bridge-ros2dds <https://github.com/eclipse-zenoh/zenoh-plugin-ros2dds>`_

.. image:: http://img.youtube.com/vi/lrFucLUWbDo/0.jpg
    :alt: Run multiple vehicles with Autoware Humble in Carla
    :target: https://youtu.be/lrFucLUWbDo

Version
-------

* Autoware: 1.5.0
* Carla: 0.9.14
* Zenoh: 1.7.2

Contents
--------

.. toctree::

   architectures
   prerequisites
   build
   usage
   faq
